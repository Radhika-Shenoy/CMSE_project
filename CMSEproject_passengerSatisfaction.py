...
flight_df = pd.read_csv('/Users/radhikavittalshenoy/Documents/flight_passengerSatisfaction.csv')
    ...
    if len(feature_options) >0:
        ...
        if plot_options == 'Scatter plot':
            ...
            if categorical_feature:
                scatterplot = sns.scatterplot(data=flight_df, x=feature_options[0], y=feature_options[1], hue=categorical_feature)
                scatterplot.set_title(f"{feature_options[0]} vs {feature_options[1]} representation")
                st.pyplot(scatterplot.figure)
        elif plot_options == 'Histogram':
            cat_feature_option = st.selectbox('Select a categorical feature', ('None','Type of Travel', 'satisfaction', 'Class', 'Gender', 'Customer Type')) 
            if cat_feature_option!='None':
                fig=px.histogram(flight_df,x=feature_options[0],color=flight_df[(cat_feature_option)],title=f"{feature_options[0]} vs {cat_feature_option}",
